...
class SingleRefControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        rel_pose_embedder_param: Optional[Dict] = None,
    ):
        super().__init__()
        logging.debug(
        ...
        # relative pose embedder
        self.pose2token = nn.Linear(rel_pose_in_dim, camera_out_dim)
        self.uncond_pose = nn.Embedding(1, 4*4)
        self.rel_pose_embedder = get_embedder(**rel_pose_embedder_param)
        ...
        self.controlnet_layout_embedding = ControlNetConditioningEmbedding(
            conditioning_embedding_channels=block_out_channels[0],
            block_out_channels=conditioning_embedding_out_channels,
            conditioning_channels=map_size[0] + bbox_embedder_param['n_classes'],
        )
    ...
    def add_cam_states(self, encoder_hidden_states, camera_emb=None):
        ...
        if camera_emb is None:
            ...
            cam_hidden_states = self.cam2token(camera_emb)  # B, N_cam, 768
        ...
        encoder_hidden_states_with_cam = torch.cat([
            ...
            repeat(encoder_hidden_states, 'b c ... -> b repeat c ...',
                   repeat=N_cam)
        ], dim=2)  # B, N_cam, 32, 768
        return encoder_hidden_states_with_cam

    def add_rel_pose_states(self, encoder_hidden_states, rel_pose, N_cam=6):
        pose_emb = self.rel_pose_embedder(rearrange(rel_pose, "b d c -> b (c d)"))
        pose_emb = repeat(pose_emb, 'b c -> b n c', n=N_cam)
        pose_emb = self.pose2token(pose_emb)  # B, N_cam, 768
        encoder_hidden_states = torch.cat([
            pose_emb.unsqueeze(2),  # B, N_cam, 1, 768
    ...
    def add_uncond_to_kwargs(
            ...
            rel_pose=None, ref_images=None, layout_canvas=None,**kwargs):
        ...
        if bboxes_3d_data is None:
            ...
            if "cam_params" in bboxes_3d_data:
                ret["bboxes_3d_data"]['cam_params'] = ret['camera_param']
        ...
        if rel_pose is not None:
            uncond_rel_pose = self.uncond_pose(torch.LongTensor(
                [0] * batch_size).to(device=self.device))
            uncond_rel_pose = uncond_rel_pose.reshape(batch_size, *rel_pose.shape[-2:])
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (self.drop_cond_ratio > 0.0 and self.training):
            ...
            bev_hdmap = bev_hdmap.type(self.dtype)
            bev_hdmap = self._random_use_uncond_map(bev_hdmap)
        else:
            uncond_mask = None
        ...
        if bboxes_3d_data is not None:
            ...
            if n_box != N_cam:
                ...
                bbox_emb = rearrange(bbox_emb, '(b n) ... -> b n ...', n=N_cam)    # [7, 6, 4, 768]
            encoder_hidden_states_with_cam = torch.cat([
                encoder_hidden_states_with_cam, bbox_emb
            ], dim=2)    # [b, N_cam, 48, 768]

        
        # 0.6. ref_image embedding
        if ref_images is not None:
            encoder_hidden_states_with_cam = torch.cat([
