...
async def process(request: Request, background_tasks: BackgroundTasks):
    global timestamp, args
    param = await request.json()# Output from traffic manager
    ...
    ##COPY from send2agent
    print("current timestamp:", timestamp)
    cur_data =  {'param': param, 'img': gen_imgs_list[0]}
...
if __name__ == '__main__':
    ...
    cfg = compose("test_config_nus")
    cfg.resume_from_checkpoint = args.resume
    pipe, weight_dtype= load_model(cfg)

    uvicorn.run(app, host=args.host, port=args.port, log_level="info")#port=11000
